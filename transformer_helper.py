...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        # SOURCE TIME STEPS
        # when encoder-decoder connected key & value have different dim to query
        time_steps = tgt_time_steps if self.self_attention else key.size(0)

        # LINEAR PROJECTION OF QUERY, KEY AND VALUE
        # key, value and query is of shape: (tgt_time_steps, batch_size, embed_dim)
        K = self.k_proj(key).contiguous().view(time_steps, batch_size, self.num_heads, self.head_embed_size)
        Q = self.v_proj(query).contiguous().view(tgt_time_steps, batch_size, self.num_heads, self.head_embed_size)
        V = self.q_proj(value).contiguous().view(time_steps, batch_size, self.num_heads, self.head_embed_size)

        # COMBINE HEADS INTO BATCH DIMENSION
        # (batch size)  * (number of heads) concatenation
        K = K.view(-1, batch_size * self.num_heads, self.head_embed_size).transpose(0, 1)
        Q = Q.view(-1, batch_size * self.num_heads, self.head_embed_size).transpose(0, 1)
        V = V.view(-1, batch_size * self.num_heads, self.head_embed_size).transpose(0, 1)

        # SCALING THE HEAD
        Q = Q / self.head_scaling
        K = K / self.head_scaling

        #DOT PRODUCT
        state = torch.bmm(Q, K.transpose(1, 2))

        #TO MASK OR NOT TO MASK? IF SO, WHAT MASK?
        if key_padding_mask is not None:
            key_padding_mask = key_padding_mask.unsqueeze(1).repeat(self.num_heads, 1, 1)
            state.masked_fill(key_padding_mask, float('-inf'))

        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(0)
            state = state + attn_mask #adding to -inf = -inf

        # SOFTMAX
        state = torch.softmax(state, dim=2)

        #DROPOUT
        state = F.dropout(state, p=self.attention_dropout, training=self.training)

        # BATCH MATRIX MULTIPLICATION WITH "VALUES" FOR H HEADS + SEPERATE HEADS
        state = torch.bmm(state, V)  # dot product with values
        #reshape to seperate out heads and head_dim
        state = state.view(batch_size, self.num_heads, -1, self.head_embed_size)
        # ensure this shape: [tgt_time_steps, batch_size, num_heads, key.size(0)]
        state = state.transpose(0,2).transpose(1,2).contiguous()

        #ATTN WEIGHTS
        attn_weights = state if need_weights else None  # reshape

        # CONCATENATION OF HEADS + OUTPUT PROJECTION
        state = state.contiguous().view(tgt_time_steps, batch_size, embed_dim)
        attn = self.out_proj(state)
        # TODO: --------------------------------------------------------------------- CUT
