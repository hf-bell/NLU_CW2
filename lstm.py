...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            self.lexical_net = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
...
##                    lexical_out = torch.tanh(torch.bmm(step_attn_weights.unsqueeze(1), src_embeddings.transpose(0,1)).squeeze(1))
                    

                    lexical_out = torch.tanh(torch.sum(step_attn_weights.unsqueeze(2)*src_embeddings.transpose(0,1), dim=1))
                    step_lexical_out = torch.tanh(self.lexical_net(lexical_out)) + lexical_out

                    lexical_contexts.append(step_lexical_out)
        ...
        if self.use_lexical_model:
            ...
            lexical_output = self.lexical_projection(lexical_output)
